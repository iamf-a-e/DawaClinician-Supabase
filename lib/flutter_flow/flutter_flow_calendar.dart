...
class _FlutterFlowCalendarState extends State<FlutterFlowCalendar> {
  ...
  Color get lightColor => widget.color.applyAlpha(0.70);

  Color get lighterColor => widget.color.applyAlpha(0.12);
  ...
  Widget build(BuildContext context) => Column(
        ...
        children: <Widget>[
          ...
          TableCalendar(
            ...
            firstDay: kFirstDay,
            lastDay: kLastDay,
            eventLoader: (day) => widget.markedDates
                    .any((markedDate) => isSameDay(markedDate, day))
                ? const [true]
                : const [],
            ...
            calendarStyle: CalendarStyle(
              defaultTextStyle: widget.dateStyle ??
                  const TextStyle(color: DawaTokens.textPrimary),
              weekendTextStyle: widget.dateStyle ??
                  const TextStyle(color: DawaTokens.textPrimary),
              holidayTextStyle: widget.dateStyle ??
                  const TextStyle(color: DawaTokens.brandPrimary),
              selectedTextStyle:
                  const TextStyle(color: Color(0xFFFAFAFA), fontSize: 16.0)
                      .merge(widget.selectedDateStyle),
              todayTextStyle: const TextStyle(
                      color: DawaTokens.brandPrimary, fontSize: 16.0)
                  .merge(widget.dateStyle),
              outsideTextStyle: const TextStyle(color: DawaTokens.textMuted)
              ...
              todayDecoration: BoxDecoration(
                color: lighterColor,
                shape: BoxShape.circle,
                border: Border.all(color: color.applyAlpha(0.35)),
              ),
            ...
            daysOfWeekStyle: DaysOfWeekStyle(
              weekdayStyle: const TextStyle(color: DawaTokens.textSecondary)
                  .merge(widget.dayOfWeekStyle),
              weekendStyle: const TextStyle(color: DawaTokens.textSecondary)
                  .merge(widget.dayOfWeekStyle),
            ),
  ...
  Widget build(BuildContext context) => Padding(
        ...
        child: InkWell(
          ...
          child: Container(
            ...
            child: Icon(
              icon.icon,
              color: icon.color ?? DawaTokens.brandPrimary,
              size: icon.size,
            ),
          ),
        ),
